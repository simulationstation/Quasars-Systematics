...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        description=(
            "Diagnostics for the selection simulation at a single W1 cut, with combined depth-map and lon-pattern injection: "
            "measure induced sin/cos lon coefficients and lambda-wedge dipoles."
        )
    )
    ap.add_argument(
        "--catalog",
    ...
    ap.add_argument("--depth-sel-scale", type=float, default=0.0)

    ap.add_argument(
        "--lon-coeffs-scan-json",
        default=None,
        help="Optional scan JSON used to inject per-cut sin/cos lon coefficients (overrides --lon-sin/--lon-cos).",
    )
    ap.add_argument("--lon-sin", type=float, default=0.0)
    ap.add_argument("--lon-cos", type=float, default=0.0)
    ap.add_argument("--lon-scale", type=float, default=1.0)
    ...
    outdir = Path(args.outdir or f"outputs/selection_sim_depthmap_plus_lon_diag_{utc_tag()}")
    ...
    if len(edges) < 2 or abs(edges[0]) > 1e-9 or abs(edges[-1] - 360.0) > 1e-9:
    ...
    if args.lon_coeffs_scan_json is not None:
        ...
        sin_coeff, cos_coeff = lon_by_cut[float(args.w1_cut)]
    else:
    ...
    sin_fit = np.array([s["lon_templates_fit"]["sin_lambda_z"] for s in samples], dtype=float)
    cos_fit = np.array([s["lon_templates_fit"]["cos_lambda_z"] for s in samples], dtype=float)
    A = np.hypot(sin_fit, cos_fit)
    phi = np.degrees(np.arctan2(sin_fit, cos_fit))
    ...
    payload: dict[str, Any] = {
        ...
        "lon_template_summary": {
            "sin_mean": float(np.mean(sin_fit)),
            "cos_mean": float(np.mean(cos_fit)),
    ...
    out_json = outdir / "selection_sim_depthmap_plus_lon_diagnostics.json"
