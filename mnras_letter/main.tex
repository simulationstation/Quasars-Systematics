...
In Supplementary Data, we also run a maximal-nuisance GLM suite at a representative cut ($W1_{\rm max}=16.6$) with a ridge-regularised extended template basis and held-out sky validation (\texttt{case\_closed\_maximal\_nuisance\_suite}; all values below are at $W1_{\rm max}=16.6$).
