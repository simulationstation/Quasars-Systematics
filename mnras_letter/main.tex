...
\title[CatWISE quasar dipole systematics]{Selection systematics in the CatWISE quasar dipole: axis drift and a calibrated epoch-slicing diagnostic}
...
Flux-limited dipole inference on a masked sky is sensitive to anisotropic depth and completeness structure near the faint selection boundary.
Using the publicly available CatWISE sample and exclusion mask, we scan cumulative faint limits $W1 \le W1_{\rm max}$ ($15.5 \le W1_{\rm max} \le 16.6$) on a fixed, coverage-defined footprint and fit dipoles using both linear regression, following the methodology of \citet{secrest2022} after removing the dominant ecliptic-latitude trend, and Poisson generalised linear models (GLM) on HEALPix counts with nuisance templates.
Across the scan, the dipole mode remains formally non-zero at $D\simeq1.6\times10^{-2}$ (clustered lognormal mocks at $W1_{\rm max}=16.6$ yield $D_{\rm hat}\approx0.0168$ with $\sigma_D\approx0.0023$), but the inferred axis drifts strongly with faint limit: its separation from the CMB dipole grows from $\sim1^\circ$ at $W1_{\rm max}=15.5$ to $\sim34^\circ$ at $W1_{\rm max}=16.6$, driven by a growing CMB-perpendicular component.
Injection/recovery tests and a correlated-cut drift Monte Carlo show that modest depth- and scan-linked systematics (including ecliptic-longitude structure) can bias both amplitude and axis under plausible template misspecification.
As a complementary diagnostic, unWISE epoch slicing (2010--2020) on the fixed Secrest-accepted CatWISE parent sample yields large apparent epoch-to-epoch variability under dipole-only recovery; however, a constrained rich parametric bootstrap with a shared dipole vector across epochs gives $P(\Delta D_{\rm sim}\ge\Delta D_{\rm obs})=0.974$ and $P(\chi^2_{\rm sim}\ge\chi^2_{\rm obs})=0.983$, showing this behaviour is expected under a constant physical dipole once measured epoch-dependent systematics are propagated through the same recovery pipeline.
We therefore treat both faint-limit scans and epoch slicing as systematics/identifiability diagnostics rather than evidence for a time-varying physical dipole.
\end{abstract}
...
More generally, catalogue dipole measurements on masked skies face a low-$\ell$ identifiability problem: anisotropic depth and selection structure can project into $\ell=1$ under the footprint and yield high-significance but template-sensitive dipole solutions.
CatWISE provides a concrete case study, and the diagnostics emphasised here (end-to-end injections, correlated-cut nulls, and calibrated epoch slicing) are broadly applicable to other flux-limited dipole claims.
...
We do not claim a closed image-level completeness solution; instead, we use constrained modelling and calibrated nulls as diagnostics of low-$\ell$ identifiability and systematics-generability under realistic survey structure.
...
Figure~\ref{fig:time_domain} shows the epoch-sliced dipole direction, amplitude, and per-epoch sample sizes for the CatWISE-parent time-domain selection.
...
Under the constrained rich bootstrap null with a shared dipole vector ($D_{\rm shared}=0.0948$), the observed dipole-only variability is not extreme: $P(\Delta D_{\rm sim}\ge\Delta D_{\rm obs})=0.974$ and $P(\chi^2_{\rm sim}\ge\chi^2_{\rm obs})=0.983$.
...
We then inject a constant true dipole through empirically fitted epoch-dependent nuisance fields and recover with dipole-only fits; in all $N_{\rm sim}=1000$ trials, both $\Delta D_{\rm sim}$ and $\chi^2_{\rm sim}$ exceed the observed values (so $P(\Delta D_{\rm sim}\ge\Delta D_{\rm obs})>0.999$ and $P(\chi^2_{\rm sim}\ge\chi^2_{\rm obs})>0.999$ at this Monte Carlo resolution).
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{Anomaly_Time_Domain.png}
\caption{\textbf{Controlled time-domain epoch-sliced diagnostic (unWISE; CatWISE parent).}
\textit{Left:} Dipole direction per epoch (colors follow time), with the CMB dipole direction marked for reference.
\textit{Top-right:} Epoch-resolved dipole amplitudes for epochs 0--15 from a dipole-only Poisson GLM fit and a vector-sum cross-check.
\textit{Bottom-right:} Per-epoch sample sizes for the fixed CatWISE parent footprint; epoch 16 is omitted due to incomplete coverage.
Although dipole-only recovery appears epoch-dependent in both amplitude and direction, Section~2.4's constrained rich bootstrap shows this variability is expected under a constant shared physical dipole once measured epoch-dependent systematics are propagated through the same recovery pipeline; the figure is therefore a systematics diagnostic rather than evidence for a time-varying dipole.}
\label{fig:time_domain}
\end{figure*}

\section{Discussion and conclusions}
Across multiple estimators on a fixed footprint, the CatWISE number-count dipole remains formally non-zero at the $D\sim10^{-2}$ level, and clustered mock catalogues imply a high \emph{formal} amplitude S/N in this diagnostic covariance treatment (not interpreted as cosmological significance).
...
We therefore do not interpret epoch variability as evidence for a time-varying physical dipole; instead it indicates that per-epoch dipole-only fits are strongly systematics-coupled and not physically interpretable as dipole measurements.

\paragraph{Conservative constrained estimates.}
At a representative cut ($W1_{\rm max}=16.6$), a ridge-regularised extended-template (``maximal-nuisance'') GLM suite with held-out sky validation (Supplementary Data: \texttt{case\_closed\_maximal\_nuisance\_suite}) shows that the inferred solution is strongly nuisance-subspace sensitive.
For example, a baseline fit gives $D=0.01678$ with axis separation $34.3^\circ$ to the CMB, while a maximal-nuisance fit gives $D=0.00578$ and $73.4^\circ$; constraining nuisance templates to be orthogonal to the monopole/dipole subspace restores $D=0.01776$ and $34.8^\circ$, illustrating strong low-$\ell$ coupling under the footprint.
...
A constrained-null parametric bootstrap with $D_{\parallel,{\rm true}}=0.0046$ yields $p_{\rm abs}\equiv P(|D_{\parallel,{\rm sim}}|\ge |D_{\parallel,{\rm obs}}|)=0.707$, indicating that the observed CMB-parallel amplitude is not anomalous under a kinematic-scale constrained null once measured systematics are propagated through the recovery pipeline.
...
As an additional systematics/identifiability check (Supplementary Data: \texttt{end\_to\_end\_completeness\_suite}), we fit a map-level pixel-count model using only catalogue-independent predictors (unWISE depth/variance, dust, stellar density, star-based W1 residuals, explicit low-$\ell$ modes, unWISE \texttt{msk} artifact proxies, and unWISE frames-table scan-angle/epoch-drift proxies).
We do not treat any external completeness proxy as a fixed multiplicative correction (i.e.\ coefficient forced to unity), because a mis-specified scaling can project into low-$\ell$ modes under the survey mask and bias dipole inference.
When external proxies are used, we include them as fit templates with regularization (and, where relevant, orthogonalization to the physical dipole mode) rather than imposing them as a hard correction.
Supplementary Data include a controlled $\gamma$-stress test (\texttt{gamma\_fixed\_vs\_fit\_stress\_test}) using an externally trained spatial completeness proxy map $T_{\rm ext}$ (Gaia qsocand) to illustrate this failure mode without asserting $\gamma\neq1$ in nature.
In Poisson injections $\mu_{\rm sim}=\mu_0\exp(\gamma_{\rm true}T_{\rm ext})$ (with $\mu_0$ from a nuisance-only fit), forcing $\gamma=1$ as a fixed offset manufactures large spurious CMB-perpendicular dipoles (e.g.\ $\langle D_\perp\rangle\simeq0.071$ at $W1_{\rm max}=16.6$ for $\gamma_{\rm true}=0.25$), while fitting $\gamma$ recovers $\gamma_{\rm true}$ and yields $\langle D_\perp\rangle\simeq0.006$.
On real data at $W1_{\rm max}=16.6$, a minimal 2-fold wedge held-out check improves test deviance by $\Delta{\rm dev}\simeq-1.5\times10^3$ when $\gamma$ is fit rather than fixed to unity.
In-sample unconstrained fits can reproduce the observed dipole and drive the residual dipole towards zero, but a refit-bootstrap shows this behaviour also occurs for injected kinematic-scale $\ell=1$ modes, demonstrating that near-zero residuals can reflect mode absorption rather than a physically meaningful ``correction''.
To test genuinely out-of-sample predictability, we also perform a cross-fit sky split (5 ecliptic-longitude wedge folds), predicting each fold from a model trained on the rest.
At $W1_{\rm max}=16.6$ the unconstrained cross-fit captures a large fraction of the observed amplitude ($D_{\rm obs}=0.0178$ vs $D_{\rm pred,oos}=0.0157$) with moderate directional agreement ($\sim16^\circ$), leaving a residual dipole $\sim5\times10^{-3}$; we interpret this as evidence that survey structure can predict much of the apparent dipole rather than a closed physical decomposition.
Enforcing a CMB-mode preservation constraint by projecting predictors orthogonal to $\{1,\hat{\bm n}\!\cdot\!\hat{\bm u}_{\rm CMB}\}$ and including a fixed kinematic-scale CMB-parallel offset leaves a large residual CMB-parallel component ($D_{\parallel,{\rm resid}}\simeq 0.011$ at $W1_{\rm max}=16.6$) that is inconsistent with a kinematic-scale constrained null ($p_{\rm abs}<10^{-3}$ for $D_{\parallel,{\rm true}}=0.0046$; $N_{\rm sim}=1000$), even with these additional artifact proxies.
These results strengthen the systematics/identifiability interpretation, but do not yet provide a validated end-to-end image-level completeness explanation for the dipole.

\clearpage

\noindent\textit{Data and code availability:} Software and reproducibility materials for this analysis are archived on Zenodo at \doi{10.5281/zenodo.18719277}; related supplementary assets used in this study are archived at \doi{10.5281/zenodo.18489200}.
